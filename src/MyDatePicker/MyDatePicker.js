...
let oneDay = 60 * 60 * 24 * 1000; //ANCHOR number of seconds in a day. 24 * 60 * 60 = 86400 sec
let todayTimestamp = Date.now() - (Date.now() % oneDay) + (new Date().getTimezoneOffset() * 1000 * 60); //The static Date.now() 
// method returns the number of milliseconds elapsed since January 1,
...
export default class MyDatePicker extends Component {
    ...
    getNumberOfDays =(year, month)=> {
        return 40 - new Date(year, month, 40).getDate(); //ANCHOR Subtract any number from 40, you'll get the date.
    }
